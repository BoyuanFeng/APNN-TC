...
#pragma unroll
    for (int tile_k = 0; tile_k+CHUNK_K < 9*CIN/128; tile_k += CHUNK_K) {
      ...
      int bit_flag = SHMEM_i / 21; // bit_flag = 0/1, indicates 
      int SHMEM_offset = SHMEM_i % 21;
...
#pragma unroll
    ...
    tmp[1].vec = *((int4*)shmem_warp_stream_ptr+21*16);
    tmp[2].vec = *((int4*)shmem_warp_stream_ptr+42*16);
    val[0].a[0] = tmp[0].a[0] + 2*tmp[1].a[0] + 4*tmp[2].a[0];
    val[0].a[1] = tmp[0].a[1] + 2*tmp[1].a[1] + 4*tmp[2].a[1];
    val[0].a[2] = tmp[0].a[2] + 2*tmp[1].a[2] + 4*tmp[2].a[2];
    val[0].a[3] = tmp[0].a[3] + 2*tmp[1].a[3] + 4*tmp[2].a[3];

    
    if (threadIdx.x < 80) {
      shmem_warp_stream_ptr = (int*)&shmem[0][0]+threadIdx.x/16*64 + (threadIdx.x%16)*4 + 16*64;
      tmp[0].vec = *((int4*)shmem_warp_stream_ptr);
      tmp[1].vec = *((int4*)shmem_warp_stream_ptr+21*16);
      tmp[2].vec = *((int4*)shmem_warp_stream_ptr+42*16);
      val[0].a[0] = tmp[0].a[0] + 2*tmp[1].a[0] + 4*tmp[2].a[0];
