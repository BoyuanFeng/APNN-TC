...
void apmm_w2a2_decompose(const int4 *W, const int4 *X, int *D, 
                                    int M_GLOBAL, int N_GLOBAL, int K_GLOBAL, int wb, int xb) {
  // GEMM configuration.
  ...
  extern __shared__ int4 shmem_w2a2[][CHUNK_K+SKEW]; // TODO: Padding opportunity may exist here.
...
#pragma unroll
    for (int tile_k = 0; tile_k < K_TILES; tile_k += CHUNK_K) {
      ...
      // the B matrix.
      int *shmem_ptr = (int*)shmem_w2a2 + warpId*16*4*(CHUNK_K+SKEW) + (laneId/4)*4*(CHUNK_K+SKEW) + laneId%4;
      ...
      *shmem_ptr = *lane_ptr;
      ...
      //   // for(int i=0; i<128; i++) {
      //     int i = 12;
      //     printf("Load from GL. i: %d, val: %x %x %x %x \n", i, *((int*)&shmem_w2a2[i][0]+0), *((int*)&shmem_w2a2[i][0]+1), *((int*)&shmem_w2a2[i][0]+2), *((int*)&shmem_w2a2[i][0]+3));
      //     i = 44;
      //     printf("Load from GL. i: %d, val: %x %x %x %x \n", i, *((int*)&shmem_w2a2[i][0]+0), *((int*)&shmem_w2a2[i][0]+1), *((int*)&shmem_w2a2[i][0]+2), *((int*)&shmem_w2a2[i][0]+3));
      //     i = 64;
      //     printf("Load from GL. i: %d, val: %x %x %x %x \n", i, *((int*)&shmem_w2a2[i][0]+0), *((int*)&shmem_w2a2[i][0]+1), *((int*)&shmem_w2a2[i][0]+2), *((int*)&shmem_w2a2[i][0]+3));
...
#pragma unroll
        for (int i = 0; i < WARP_COL_TILES; i++) {
          ...
          const int4 *tile_ptr = &shmem_w2a2[shmem_idx_a][k_step];
...
#pragma unroll
          for (int j = 0; j < WARP_ROW_TILES; j++) {
            if (i == 0) {
              ...
              size_t shmem_idx_b = shmem_idx_b_off +
                                   ...
                                   (j * N);
              const int4 *tile_ptr = &shmem_w2a2[shmem_idx_b][k_step * (K/128)];
    ...
    int *shmem_warp_tile_ptr = (int*)&shmem_w2a2[0][0] +
                              (warpId / 2) * 64 * 16 +
...
#pragma unroll
    ...
    //   for(int i=12; i<13; i++) {
    //     for(int j=0; j<64; j++) {
    //       printf("i: %d, j: %d, val: %d\n", i, j, *((int*)&shmem_w2a2[0][0]+i*64+j));
    //     }
    //     for(int j=0; j<64; j++) {
    //       printf("i: %d, j: %d, val: %d\n", i+32, j, *((int*)&shmem_w2a2[0][0]+(i+32)*64+j));
    //     }
    //   }
    // }


    int *shmem_warp_stream_ptr = (int*)&shmem_w2a2[0][0] + warpId*64 + laneId;
    int mask = 1;
    int bit0, bit1;
    unsigned r0, r1;
...
void compute_ref_pack(int4 *W, int4 *X, int *ref_C, int M_GLOBAL, int N_GLOBAL, int K_GLOBAL, int W_BIT, int X_BIT, int OUT_BIT) {
  ...
  for(int m=0; m<M_GLOBAL; m++) {
    for(int n_tile=0; n_tile<N_GLOBAL/32; n_tile++) {
      ...
      for(int n=0; n<32; n++) {
        int tmp = C_ref_before_decompose[m*N_GLOBAL+n_tile*32+n];
      ...
      for(int b=0; b<OUT_BIT; b++) {
        ref_C[b*M_GLOBAL*N_GLOBAL/32+m*N_GLOBAL/32+n_tile] = val[b];
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  compute_ref_pack(W_h, X_h, Output_ref, M_GLOBAL, N_GLOBAL, K_GLOBAL, W_BIT, X_BIT, X_BIT);

  /* validation results */
  validate_results_pack(Output_h, Output_ref, M_GLOBAL, N_GLOBAL, X_BIT);
  free(W_h);
  free(X_h);
  free(Output_h);
  free(Output_ref);
