...
#pragma unroll
    ...
    size_t idx = warpId * 8 * 64 + (laneId%16) * 4 + (laneId/16)*64;
...
#pragma unroll
    for (int i = 0; i < 4; i++) {
      ...
      shmem_warp_stream_ptr += 2*64;
    }

    __syncthreads();

    idx = warpId * 8 * 32 + (laneId%16) * 2 + (laneId/16)*32;
...
#pragma unroll
    ...
    size_t gmem_idx = block_tile_i*M*N_GLOBAL/2 + block_tile_j*N/2 + warpId*4*N_GLOBAL/2 + (laneId%8)*4 + (laneId/8)*N_GLOBAL/2;
  ...
  #pragma unroll
    for (int i = 0; i < 2; i++) {
      *((int4 *)(dst_gmem_warp_stream_ptr + i*32*N_GLOBAL/2)) =
      *((int4 *)(shmem_warp_stream_ptr + i*32*32));
    }
