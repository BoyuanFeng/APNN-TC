...
__global__ void compute_gemm_imma(const int4 *A, const int4 *B, int *D) {
  ...
  for (unsigned int block_pos = blockIdx.x;; block_pos += gridDim.x) {
    ...
    const int4 *warp_ptr = (warpId < 1) ? (&A[block_tile_i * M * (K_GLOBAL/128)])
...
#pragma unroll
    ...
    int *shmem_warp_tile_ptr = (int*)&shmem[0][0] +
                              warpId * SHMEM_STRIDE * M * 2; // Will be used only when writing back D. May be moved outside the for loop. TODO.
...
#pragma unroll
    ...
    const size_t idx = warpId * SHMEM_STRIDE * 16 + laneId*4;
    int *shmem_warp_stream_ptr = (int*)&shmem[0][0]+idx;

    // This warp's pointer to the C matrix data to copy memory from to shared memory. 
    // TODO: May be moved outside the for loop.
    size_t gmem_idx = block_tile_i * M * GLOBAL_MEM_STRIDE + block_tile_j*N + warpId*16*GLOBAL_MEM_STRIDE + laneId/8 * GLOBAL_MEM_STRIDE + laneId%8 * 4;
