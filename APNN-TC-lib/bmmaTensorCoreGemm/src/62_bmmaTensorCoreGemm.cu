...
__global__ void compute_conv_imma(const int4 *W, const int4 *X, int *Output, int Height, int Width, int CIN, int COUT) {
  ...
  for (unsigned int block_pos = blockIdx.x;; block_pos += gridDim.x) {
    const unsigned int block_i = (block_pos/(COUT/128)) / (Width/5) * 5;
    const unsigned int block_j = (block_pos/(COUT/128)) % (Width/5) * 5;
...
#pragma unroll
    for (int tile_k = 0; tile_k+CHUNK_K < 9*CIN/128; tile_k += CHUNK_K) {
      ...
      int bit_flag = SHMEM_i / 25;
      int SHMEM_offset = SHMEM_i % 25;
...
#pragma unroll
    ...
    int *shmem_warp_stream_ptr = (int*)&shmem[0][0]+threadIdx.x/32*3*128 + (threadIdx.x%32)*4;
#pragma unroll
    for(int i = 0; i < 3; i++) {
      tmp0.vec = *((int4*)shmem_warp_stream_ptr);
      tmp1.vec = *((int4*)shmem_warp_stream_ptr+25*32);
      tmp2.vec = *((int4*)shmem_warp_stream_ptr+50*32);
      tmp3.vec = *((int4*)shmem_warp_stream_ptr+75*32);
      tmp4.vec = *((int4*)shmem_warp_stream_ptr+100*32);
      val[i].a[0] = tmp0.a[0] + 2*tmp1.a[0] + 4*tmp2.a[0] + 8*tmp3.a[0] + 16*tmp4.a[0];
      val[i].a[1] = tmp0.a[1] + 2*tmp1.a[1] + 4*tmp2.a[1] + 8*tmp3.a[1] + 16*tmp4.a[1];
      val[i].a[2] = tmp0.a[2] + 2*tmp1.a[2] + 4*tmp2.a[2] + 8*tmp3.a[2] + 16*tmp4.a[2];
      val[i].a[3] = tmp0.a[3] + 2*tmp1.a[3] + 4*tmp2.a[3] + 8*tmp3.a[3] + 16*tmp4.a[3];
    ...
    int SHMEM_row = threadIdx.x/32*3;
