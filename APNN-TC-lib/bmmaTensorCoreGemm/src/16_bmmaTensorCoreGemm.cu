...
__global__ void compute_gemm_imma(const int4 *A, const int4 *B, int *D) {
  ...
  for (unsigned int block_pos = blockIdx.x;; block_pos += gridDim.x) {
    ...
    const int4 *warp_ptr = (warpId < 2) ? (&A[block_tile_i * M * (K_GLOBAL/128)] +
                                              M * (K_GLOBAL/128) * 2 * (warpId % 2))
                                           : (&B[block_tile_j * N * (K_GLOBAL/128)] +
                                              N * (K_GLOBAL/128) * 2 * (warpId % 2));
    ...
    for (int tile_k = 0; tile_k < K_TILES; tile_k += CHUNK_K) {
      ...
      size_t shmem_idx =
          warpId < (WARPS_PER_BLOCK / 2)
              ? (2 * M * (warpId % (WARPS_PER_BLOCK / 2)))
              : (2 * N * (warpId % (WARPS_PER_BLOCK / 2)) + shmem_idx_b_off);
...
#pragma unroll
      for (int i = 0; i < (16 / CHUNK_COPY_LINES_PER_WARP); i++) {
...
#pragma unroll
    ...
    const size_t idx = warpId * SHMEM_STRIDE * 8 + laneId*4;
    int *shmem_warp_stream_ptr = (int*)&shmem[0][0]+idx;

    // This warp's pointer to the C matrix data to copy memory from to shared memory. 
    // TODO: May be moved outside the for loop.
    size_t gmem_idx = block_tile_i * M * GLOBAL_MEM_STRIDE + block_tile_j*N + warpId*8*GLOBAL_MEM_STRIDE + laneId/8 * GLOBAL_MEM_STRIDE + laneId%8 * 4;
...
#pragma unroll
    for (int i = 0; i < 2; i++) {
      *((int4 *)(dst_gmem_warp_stream_ptr + GLOBAL_MEM_STRIDE * i * 4)) =
      *((int4 *)(shmem_warp_stream_ptr + i*4*SHMEM_STRIDE));
    }
