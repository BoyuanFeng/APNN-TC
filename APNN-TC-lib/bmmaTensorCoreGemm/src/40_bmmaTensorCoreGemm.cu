...
__global__ void apmm_w5a1(const int4 *A, const int4 *B, int *D, int M_GLOBAL, int N_GLOBAL, int K_GLOBAL, int xb, int wb) {
  // GEMM configuration.
...
#pragma unroll
    ...
    size_t idx = warpId * 5 * 128 + laneId * 4;
...
#pragma unroll
    for (int i = 0; i < 3; i++) {
      ...
      val[4*i] = tmp0.a[0] + 2*tmp1.a[0] + 4*tmp2.a[0] + 8*tmp3.a[0] + 16*tmp4.a[0];
      val[4*i+1] = tmp0.a[1] + 2*tmp1.a[1] + 4*tmp2.a[1] + 8*tmp3.a[1] + 16*tmp4.a[1];
      val[4*i+2] = tmp0.a[2] + 2*tmp1.a[2] + 4*tmp2.a[2] + 8*tmp3.a[2] + 16*tmp4.a[2];
      val[4*i+3] = tmp0.a[3] + 2*tmp1.a[3] + 4*tmp2.a[3] + 8*tmp3.a[3] + 16*tmp4.a[3];

      shmem_warp_stream_ptr += 40*128;
    }
    ...
    if (warpId<1) {
      ...
      val[12] = tmp0.a[0] + 2*tmp1.a[0] + 4*tmp2.a[0] + 8*tmp3.a[0] + 16*tmp4.a[0];
      val[13] = tmp0.a[1] + 2*tmp1.a[1] + 4*tmp2.a[1] + 8*tmp3.a[1] + 16*tmp4.a[1];
      val[14] = tmp0.a[2] + 2*tmp1.a[2] + 4*tmp2.a[2] + 8*tmp3.a[2] + 16*tmp4.a[2];
      val[15] = tmp0.a[3] + 2*tmp1.a[3] + 4*tmp2.a[3] + 8*tmp3.a[3] + 16*tmp4.a[3];
    }
...
#pragma unroll
    for(int i = 0; i < 3; i++) {
      *(shmem_warp_stream_ptr+0) = val[4*i+0];
      *(shmem_warp_stream_ptr+1) = val[4*i+1];
      *(shmem_warp_stream_ptr+2) = val[4*i+2];
      *(shmem_warp_stream_ptr+3) = val[4*i+3];
      shmem_warp_stream_ptr += 8*128;
    }
    if(warpId<1) {
      *(shmem_warp_stream_ptr+0) = val[12];
      *(shmem_warp_stream_ptr+1) = val[13];
      *(shmem_warp_stream_ptr+2) = val[14];
      *(shmem_warp_stream_ptr+3) = val[15];
    }
    __syncthreads();
    ...
    size_t gmem_idx = block_tile_i*M/5*N_GLOBAL + block_tile_j*N + warpId*N_GLOBAL + laneId*4;
...
#pragma unroll
    for (int i = 0; i < 3; i++) {
      *((int4 *)(dst_gmem_warp_stream_ptr + i*8*N_GLOBAL)) =
      *((int4 *)(shmem_warp_stream_ptr + i*8*128));
    }
    if(warpId<1) {
      *((int4 *)(dst_gmem_warp_stream_ptr + 4*8*N_GLOBAL)) =
      *((int4 *)(shmem_warp_stream_ptr + 4*8*128));
    }
    __syncthreads();
  }
}
...
// #endif
    ...
    printf("V40, 128x128. M_GLOBAL: %d, N_GLOBAL: %d, K_GLOBAL: %d, X_BIT: %d, W_BIT: %d\n", M_GLOBAL, N_GLOBAL, K_GLOBAL, X_BIT, W_BIT);
    printf("Time: %f ms\n", bmma_ms_avg);  
