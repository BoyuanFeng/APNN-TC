...
#pragma unroll
    ...
    size_t gmem_idx = image_starting_idx;
    
    // Now that shared memory contains all the D tiles, stream them to global memory.
    int *dst_gmem_warp_stream_ptr = &Output[gmem_idx];
...
#pragma unroll
      for (int j = 0; j < WARP_ROW_TILES; j++) {
        int *tile_ptr = dst_gmem_warp_stream_ptr + i * Width * M + j * N;
        wmma::store_matrix_sync(tile_ptr, c[i][j], Width, wmma::mem_row_major);
      }
    }
