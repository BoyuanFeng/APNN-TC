...
#pragma unroll
    ...
    size_t idx = warpId * 8 * 64 + (laneId%8) * 8 + (laneId/8)*64;
...
#pragma unroll
    ...
    if (threadIdx.x < 128) {
      ...
      size_t gmem_idx = block_tile_i*M*N_GLOBAL/8 + block_tile_j*N/8 + (threadIdx.x/2)*N_GLOBAL/8 + (threadIdx.x%2)*4;
...
// #endif
    ...
    printf("V34, 64x64. M_GLOBAL: %d, N_GLOBAL: %d, K_GLOBAL: %d, X_BIT: %d, W_BIT: %d\n", M_GLOBAL, N_GLOBAL, K_GLOBAL, X_BIT, W_BIT);
    printf("Time: %f ms\n", bmma_ms_avg);  
