...
void apmm_w2a8(const int4 *W, const int4 *X, int *D, 
  ...
  extern __shared__ int4 shmem_w2a8[][CHUNK_K+SKEW]; // TODO: Padding opportunity may exist here.
  ...
  for (unsigned int block_pos = blockIdx.x;; block_pos += gridDim.x) {
    const unsigned int block_tile_i = block_pos / (N_GLOBAL/8) * 32;
...
#pragma unroll
    for (int tile_k = 0; tile_k < K_TILES; tile_k += CHUNK_K) {
      ...
      // the B matrix.
      int *shmem_ptr = (int*)shmem_w2a8 + warpId*16*4*(CHUNK_K+SKEW) + (laneId/4)*4*(CHUNK_K+SKEW) + laneId%4;
      ...
      *shmem_ptr = *lane_ptr;
      ...
      // if (warpId == 0 && laneId == 0 && blockIdx.x==0) {
      //   for(int i=0; i<128; i++) {
      //     printf("Load from GL. i: %d, val: %x %x %x %x \n", i, *((int*)&shmem_w2a8[i][0]+0), *((int*)&shmem_w2a8[i][0]+1), *((int*)&shmem_w2a8[i][0]+2), *((int*)&shmem_w2a8[i][0]+3));
      //   }
      // }
...
#pragma unroll
        for (int i = 0; i < WARP_COL_TILES; i++) {
          ...
          const int4 *tile_ptr = &shmem_w2a8[shmem_idx_a][k_step];
...
#pragma unroll
          for (int j = 0; j < WARP_ROW_TILES; j++) {
            if (i == 0) {
              ...
              size_t shmem_idx_b = shmem_idx_b_off +
                                   ...
                                   (j * N);
              const int4 *tile_ptr = &shmem_w2a8[shmem_idx_b][k_step * (K/128)];
    ...
    int *shmem_warp_tile_ptr = (int*)&shmem_w2a8[0][0] +
                              (warpId / 2) * 64 * 16 +
...
#pragma unroll
    ...
    //   for(int i=62; i<64; i++) {
    //     for(int j=0; j<64; j++) {
    //       printf("i: %d, j: %d, val: %d\n", i, j, *((int*)&shmem_w2a8[0][0]+i*64+j));
    //     }
    //   }
    // }

    // This pointer is used to stream the C and D matrices block-wide tile to and from shared memory.
    // int *shmem_warp_stream_ptr = (int*)&shmem_w2a8[0][0] + warpId * SHMEM_STRIDE * M; // Will be used only when writing back D. Maybe moved outside the for loop. TODO.
    size_t idx = threadIdx.x/8 * 64 + threadIdx.x%8;
    int *shmem_warp_stream_ptr = (int*)&shmem_w2a8[0][0]+idx;
...
#pragma unroll
    for(int j=0; j<8; j++) {
      int tmp = *(shmem_warp_stream_ptr+8*j+32*64);
      val += (multiplier*tmp);
      multiplier *= 2;
    }
...
int main(int argc, char **argv) {
  ...
  for (int M_GLOBAL=128; M_GLOBAL<=1024; M_GLOBAL += 128 ) {
    ...
    checkCudaErrors(cudaFuncSetAttribute(
      apmm_w2a8, cudaFuncAttributeMaxDynamicSharedMemorySize,
      SHMEM_SZ));
    ...
    for(int iter=0; iter<NUM_PROFILES; ++iter){
            ...
            checkKernelErrors(
              (apmm_w2a8<<<deviceProp.multiProcessorCount, THREADS_PER_BLOCK,
    ...
    printf("V76, 64x64. M_GLOBAL: %d, N_GLOBAL: %d, K_GLOBAL: %d, X_BIT: %d, W_BIT: %d\n", M_GLOBAL, N_GLOBAL, K_GLOBAL, X_BIT, W_BIT);
    printf("Time: %f ms\n", bmma_ms_avg);  
