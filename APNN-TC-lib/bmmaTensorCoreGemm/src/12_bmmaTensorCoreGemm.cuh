...
void compute_gemm_imma(const int4 *A, const int4 *B, int *D, int Mtiles, int Ntiles, int Ktiles) {
  ...
  for (unsigned int block_pos = blockIdx.x;; block_pos += gridDim.x) {
    const unsigned int block_tile_i = block_pos / (Ntiles/8) * 8;
    const unsigned int block_tile_j = block_pos % (Ntiles/8) * 8;

    // Stop when there are no more D matrix tiles to compute in this CTA.
    if (block_tile_i >= Mtiles) {
      break;
    }
...
#pragma unroll
    for (int tile_k = 0; tile_k < Ktiles; tile_k += CHUNK_K) {
