...
__global__ void apmm_w1a8(const int4 *A, const int4 *B, int *D, int M_GLOBAL, int N_GLOBAL, int K_GLOBAL, int xb, int wb) {
  // GEMM configuration.
...
#pragma unroll
    ...
    size_t idx = warpId * 16 * 128 + laneId * 8;
...
#pragma unroll
    for (int i = 0; i < 8; i++) {
      ...
      shmem_warp_stream_ptr += 128*2;
    }

    __syncthreads();

    idx = warpId * 16 * 16 + laneId;
...
#pragma unroll
    ...
    shmem_warp_stream_ptr = (int*)&shmem[0][0]+warpId * 8 * 16 + laneId*4;

    // This warp's pointer to the C matrix data to copy memory from to shared memory. 
    // TODO: May be moved outside the for loop.
    size_t gmem_idx = block_tile_i*M*N_GLOBAL/8 + block_tile_j*N/8 + warpId*8*N_GLOBAL/8 + (laneId%4)*4 + (laneId/4)*N_GLOBAL/8;
...
#pragma unroll
    for (int i = 0; i < 2; i++) {
      *((int4 *)(dst_gmem_warp_stream_ptr + 64*i*N_GLOBAL/4)) =
      *((int4 *)(shmem_warp_stream_ptr + i*64*16));
    }
...
int main(int argc, char **argv) {
  ...
  for (int M_GLOBAL=128; M_GLOBAL<=1024; M_GLOBAL += 128 ) {
    ...
    checkCudaErrors(cudaMalloc(reinterpret_cast<void **>(&Output), sizeof(int) * M_GLOBAL * N_GLOBAL* X_BIT));
...
// #endif
    ...
    printf("V35, 128x128. M_GLOBAL: %d, N_GLOBAL: %d, K_GLOBAL: %d, X_BIT: %d, W_BIT: %d\n", M_GLOBAL, N_GLOBAL, K_GLOBAL, X_BIT, W_BIT);
    printf("Time: %f ms\n", bmma_ms_avg);  
