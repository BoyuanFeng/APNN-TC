...
__global__ void compute_conv_imma(const int4 *W, const int4 *X, int *Output, int Height, int Width, int CIN, int COUT) {
  ...
  for (unsigned int block_pos = blockIdx.x;; block_pos += gridDim.x) {
    const unsigned int block_i = (block_pos/(COUT/32)) / (Width/3) * 3;
    const unsigned int block_j = (block_pos/(COUT/32)) % (Width/3) * 3;
...
#pragma unroll
    for (int tile_k = 0; tile_k+CHUNK_K < 9*CIN/128; tile_k += CHUNK_K) {
      ...
      int row = SHMEM_offset / 3;
      int col = SHMEM_offset % 3;
      int t = threadIdx.x % 4;
...
#pragma unroll
    for (int i=0; i<6; i++) {
      tmp[2*i] = *(shmem_warp_stream_ptr+10*i*64);
      tmp[2*i+1] = *(shmem_warp_stream_ptr+10*i*64+32);
    }

#pragma unroll
    for(int i=0; i<6; i++) {
      val[0] += tmp[2*i]*multiplier;
      multiplier*=2;
      val[0] += tmp[2*i+1]*multiplier;
    }
...
#pragma unroll
      for(int i=0; i<6; i++) {
        val[1] += tmp[2*i]*multiplier;
        multiplier*=2;
        val[1] += tmp[2*i+1]*multiplier;
      }
    }
    ...
    //     + SHMEM_col*4);
    // }
    *dst_gmem_warp_stream_ptr = val[0];
    ...
    if (threadIdx.x < 64) {
      ...
      dst_gmem_warp_stream_ptr = Output + block_i * Width * COUT + block_j*COUT + block_z 
              + Output_row*Width*COUT + Output_col*COUT
              + SHMEM_col;
    *dst_gmem_warp_stream_ptr = val[1];
    }
