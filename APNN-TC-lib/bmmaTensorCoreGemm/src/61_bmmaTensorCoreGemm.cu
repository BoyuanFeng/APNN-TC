...
#pragma unroll
    ...
    if (threadIdx.x < 192) {
      ...
      tmp[1].vec = *((int4*)shmem_warp_stream_ptr+12*16);
      tmp[2].vec = *((int4*)shmem_warp_stream_ptr+24*16);
      tmp[3].vec = *((int4*)shmem_warp_stream_ptr+36*16);
      tmp[4].vec = *((int4*)shmem_warp_stream_ptr+48*16);
      val.a[0] = tmp[0].a[0] + 2*tmp[1].a[0] + 4*tmp[2].a[0] + 8*tmp[3].a[0] + 16*tmp[4].a[0];
      val.a[1] = tmp[0].a[1] + 2*tmp[1].a[1] + 4*tmp[2].a[1] + 8*tmp[3].a[1] + 16*tmp[4].a[1];
      val.a[2] = tmp[0].a[2] + 2*tmp[1].a[2] + 4*tmp[2].a[2] + 8*tmp[3].a[2] + 16*tmp[4].a[2];
      val.a[3] = tmp[0].a[3] + 2*tmp[1].a[3] + 4*tmp[2].a[3] + 8*tmp[3].a[3] + 16*tmp[4].a[3];  
    }
