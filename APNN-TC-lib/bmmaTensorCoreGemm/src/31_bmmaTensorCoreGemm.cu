...
__global__ void apmm_w1a2(const int4 *A, const int4 *B, int *D, int M_GLOBAL, int N_GLOBAL, int K_GLOBAL, int xb, int wb) {
  // GEMM configuration.
  M_GLOBAL = M_GLOBAL * xb;
  N_GLOBAL = N_GLOBAL * wb;
...
#pragma unroll
    for (int i = 0; i < 16; i++) {
      ...
      val[2*i] = tmp0.a[0] + 2*tmp0.a[1];
      val[2*i+1] =  tmp0.a[2] + 2*tmp0.a[3];
...
#pragma unroll
    ...
    shmem_warp_stream_ptr = (int*)&shmem[0][0]+warpId * 64 * 16 + laneId*4;

    // This warp's pointer to the C matrix data to copy memory from to shared memory. 
    // TODO: May be moved outside the for loop.
    size_t gmem_idx = block_tile_i*M*N_GLOBAL/2 + block_tile_j*N/2 + warpId*16*N_GLOBAL/2 + (laneId%16)*4 + (laneId/16)*N_GLOBAL/2;
...
void init_matrices(int4 *X, int4 *W, int M_GLOBAL, int N_GLOBAL, int K_GLOBAL, int X_BIT, int W_BIT){
  int *X_int = (int*) X;
  int *W_int = (int*) W;
  for(int b = 0; b<X_BIT; b++) {
    for(int i = 0; i < M_GLOBAL; i++) {
      for(int j = 0; j < K_GLOBAL/32; j++) {
        ...
        X_int[b*M_GLOBAL*K_GLOBAL/32 + i*K_GLOBAL/32+j] = rand();
      }
    }  
  }
  ...
  for(int b=0; b<W_BIT; b++) {
    for(int i = 0; i < N_GLOBAL; i++) {
      for(int j = 0; j < K_GLOBAL/32; j++) {
        ...
        W_int[b*N_GLOBAL*K_GLOBAL/32 + i*K_GLOBAL/32+j] = rand();
      }
    }  
  }
}
...
// #endif
    ...
    printf("V31, 128x128. M_GLOBAL: %d, N_GLOBAL: %d, K_GLOBAL: %d, X_BIT: %d, W_BIT: %d\n", M_GLOBAL, N_GLOBAL, K_GLOBAL, X_BIT, W_BIT);
    printf("Time: %f ms\n", bmma_ms_avg);  
