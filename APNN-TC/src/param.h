...
void filter_quantization(uin32* bit_filter_gpu, float* filter_gpu,
                            ...
                            int in_channel, int bit=2);

void weight_quantization(uin32*bit_weight_gpu, float* weight_gpu, 
                        int in_channel, int out_channel, int bit=1);
...
class Fc128LayerParam
{
    public:
        Fc128LayerParam(const char* name, int _input_height, int _input_width, 
                int _weight_width, int a_bit=2, int w_bit=1) : 
        ...
        // int weight_bit_bytes() { return _w_bit*weight_bit_size()*sizeof(uin128);}
        int weight_bit_bytes() { return _w_bit*PAD128(weight_height)*weight_width/32*sizeof(uin32);}
...
class Out128LayerParam
{
    public:
        Out128LayerParam(const char* name, int _input_height, 
                int _input_width, int _weight_width, int a_bit=2, int w_bit=1) :
            input_height(_input_height), input_width(_input_width),
            weight_height(_input_width), weight_width(_weight_width),
            output_height(_input_height), output_width(_weight_width),
            input(NULL), input_gpu(NULL), output(NULL), output_gpu(NULL),
        ...
        int output_bit_bytes() { return _a_bit*output_bit_size()*sizeof(float);}
        ...
        Out128LayerParam* initialize(FILE* config_file, uin32* prev_layer_gpu)
        {
            ...
            // PackFcWeight128<<<numBlocksPerSm*deviceProp.multiProcessorCount, numThreads>>>(
            //         weight_float, weight_gpu, weight_height, weight_width);
            weight_quantization(weight_gpu, weight_float, weight_height, weight_width, _w_bit);
...
class Conv128LayerParam
{
    public:
        Conv128LayerParam(const char* name, int _input_height, int _input_width, 
                ...
                int _pool_height=1, 
                int _pool_width=1, 
                bool _ahead_fc=false, 
                bool _save_residual=false,
                bool _inject_residual=false, 
                int _residual_channels=0,
                bool _residual_pool=false, 
                int a_bit=2, int w_bit=1):
                ...
                int* input_residual_gpu = NULL)
        {
            ...
            filter_quantization(filter_gpu, filter_float, output_channels, filter_height, filter_width, 
                                input_channels, _w_bit);
